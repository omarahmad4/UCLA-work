			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	int i, j;
   	int index_X, index_Y;
	int max_size = X*Y*Z;
							//need to parallelize and lock and release for the += part only 
							//calc random seperately
...
//#pragma omp parallel for 
   		for(j = 0; j < Ones; j++) {
			//#pragma omp critical
			{
				double tempProb = (pow((array[index[i*Ones + j]] - 100),2) -
   							  pow((array[index[i*Ones + j]]-228),2))/50.0;

				{
			probability[i] += tempProb;//(pow((array[index[i*Ones + j]] - 100),2) - pow((array[index[i*Ones + j]]-228),2))/50.0;
				}
			}
		}
...
//#pragma omp parallel for
	for(i = 0; i < n; i++)
	{
		test = exp(probability[i]);
   		...
   		weights[i] *= test;//= weights[i] * exp(probability[i]);
	}
