...
int* calcDivide(int num, int denom)//, int results[])
{
  
  int i;
  int div = num/denom;
  int mod = num % denom;
  int* results = malloc((div+1)*sizeof(int));
  //    new int[div+1];// = {0};//delete eventually
  if(mod == 0)
    {
      for(i = 0; i < denom; i++)
	results[i] = div;
...
main( int argc, char **argv )
{
    ...
    for (i = 0; i < nthreads; i++)
      {
	ret = pthread_create(&threads[i], NULL, func1, NULL);
