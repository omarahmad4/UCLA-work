...
void *func1(void* Vdata)
{
  ...
  //      printf(" %i\t %i\n", data->minmax[2*index], data->minmax[2*index+1]);  
  ...
  //    for( int px=0; px<width; px++ )
	...
	//	      for( int py=data->minmax[index]; py<(data->minmax[index+1]-1); ++py )
	//int py = 5;
	//for(; py < 0; py++)
	{

	  //THREADING CANDIDDATE 1
	   ...
	   */
	  //	  data->threadData[px][py][0] = scaled_color[0];
	  ...
	  for(int i = 0; i < 3; i ++)
	    {
	      	  ...
	      	  data->threadData[px][py][i] = scaled_color[i];
		  //		  printf("%.0f ", data->threadData[px][py][i]);
	    }
	  //  printf("\n");
	  ...
	  for(int i = 0; i < 3; i ++)
		...
		//	  printf("%.0f ", abc.threadData[px][py][i]);
	    }
	  //  printf("\n");
	  ...
	  //	   printf("%.0f %.0f %.0f\n",
	  // 		 data->threadData[px][py][0],
	  // 		 data->threadData[px][py][1],
	  // 		 data->threadData[px][py][2]);
	  //	  printf( "%.0f %.0f %.0f\n",
	  //		  scaled_color[0], scaled_color[1], scaled_color[2] );
        }
      //            printf( "\n" );
...
main( int argc, char **argv )
{
  ...
  // printf("%i\t", abc.minmax[i]);


  for(i = 0; i < nthreads; i++)
    {
                  pthread_create(&threads[i], NULL, &func1, (void *) &abc);
      //      printf("%i\t", threads[i]);
      //IF RETURN ERROR
    }
