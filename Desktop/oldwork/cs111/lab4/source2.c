...
int main(int argc, char **argv)
{
  ...
  if(SSL_library_init() < 0)
    errorExit("initializing ssl library", 1, 0);
  ...
  if(context == NULL)
    errorExit("initializing context", 1, 0);
  ...
  if(SSL_connect(ssl) != 1)
    errorExit("connecting to ssl", 1, 0);


  char idBuf [32];
  sprintf(idBuf, "ID=%d\n", id);
  SSL_write(ssl, idBuf, strlen(idBuf));
