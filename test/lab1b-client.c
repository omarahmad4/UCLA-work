...
int errno, status, cpid, i; //global bc used in multiple functions
...
int main (int argc, char **argv)
{
  ...
  if(isLog)
    {
      logFD = open(log, O_RDWR | O_APPEND | O_CREAT, 0666);
      if(logFD == -1)
	{
  ...
  //BELOW THIS LINE LIKELY DELETE ALL PIPE STUFF
      ...
      else if (pollret > 0)
	{
	  if(pollfds[0].revents & POLLIN) //read from keyboard input
	      ...
	      if(isLog)
		{
	      ...
	      if(isCompress)
		{		  
		  char compData[4000]; //not sure how big needs to be - how good is compresssion?
	  ...
	  if(pollfds[1].revents & POLLIN) //read from socket
	      ...
	      if(isCompress)
		{
		  char decompData[256];
		  int cret;
		  cret = myinf(buffer, nBytes, decompData, 256);
		  char decompBuf[512];
		  int index = 0;
